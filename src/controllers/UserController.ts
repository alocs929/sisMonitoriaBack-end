...
class UserController {
  ...
  async search(req:Request , res:Response){
    ...
    const classesScheduleRepository = getCustomRepository(ClassesScheduleRepository);

    const { subject } = req.params;

    const  Classes  = await classesRepository.find({subject: subject });
    var total = []
    for (let i = 0; i < Classes.length; i++) {
      const monitor  = await monitorRepository.find({id: Classes[i].monitor_id})
      const classesschedule  = await classesScheduleRepository.find({classe_id: Classes[i].id})
      const classes = Classes[i]
      const dados = {
        monitor,classes  , classesschedule
      }
      total.push(dados)
    }
    res.json(total)
